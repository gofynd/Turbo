...
function BankForm({
  ...
  const validateAccounHolder = (value) => {
    if (!value || value.trim().length === 0) {
      return t("resource.refund.account_holder_required");
    }

    if (/\d/.test(value)) {
      return t("resource.refund.numbers_not_allowed_in_account_holder_name");
    }

    // Add validation for special characters (except spaces and common name characters)
    if (!/^[a-zA-Z\s.',-]+$/.test(value)) {
      return t("resource.refund.special_characters_not_allowed_in_account_holder_name");
    }

    // Minimum name length check
    if (value.trim().length <= 5) {
      return t("resource.refund.account_holder_name_should_be_more_than_5_characters");
    }

    // Maximum name length check (assuming a reasonable max)
    if (value.trim().length > 50) {
      return t("resource.refund.account_holder_name_should_not_exceed_50_characters");
    }
  ...
  const validateAccountNo = (value) => {
    if (!value || value.toString().trim().length === 0) {
      return t("resource.refund.account_number_is_required");
    }
    ...
    if (!/^\d+$/.test(accountNumber)) {
      return t("resource.refund.account_number_should_contain_only_numbers");
    }

    // Check minimum length (typical minimum for most banks)
    if (accountNumber.length < 9) {
      return t("resource.refund.account_number_should_be_at_least_9_digits");
    }

    // Check maximum length (typical maximum for most banks)
    if (accountNumber.length > 18) {
      return t("resource.refund.account_number_should_not_exceed_18_digits");
    }
    ...
    <div className={styles.formContainer}>
      ...
      >
        <div className={styles.formItem}>
          <Input
            label={t("resource.refund.ifsc_code")}
        ...
        <div className={styles.formItem}>
          <Input
            label={t("resource.refund.account_number")}
            labelVariant="floating"
        ...
        <div className={styles.formItem}>
          <Input
            ...
            {...register("confirmedAccountNo", {
              validate: (value) =>
                value === getValues("accountNo") || t("resource.refund.account_numbers_do_not_match"),
            })}
